...
<details>
<summary><strong>⊞ Windows (WSL) Instructions</strong></summary>

Follow these steps to set up Docker and run Wilson's simulation:
(Tested on Windows 10 with WSL2, latest Docker)
### Prerequisites Setup

**⚠️ Important:** Install Windows Subsystem for Linux (WSL) and enable a Linux distribution before proceeding. See the [WSL install guide](https://learn.microsoft.com/en-us/windows/wsl/install). Download and start docker engine https://www.docker.com/products/docker-desktop/ . You will also need root/sudo access inside the WSL distribution to complete the steps below.
...
2. **Update system packages and install prerequisites:**
   ```bash
   sudo apt update
   sudo apt install -y git curl x11-xserver-utils x11-utils x11-xkb-utils
   ```

3. **Clone Wilson repository:**
   ```bash
   git clone https://github.com/tracelarue/wilson.git
   cd wilson
   ```

4. **Create API key file** (for AI voice/text commands):
   Create a `.env` file in the wilson directory with your Google API key:
   ```.env
   GOOGLE_API_KEY="your_api_key_here"
   ```
...
5. **Start Docker:**
   Start the docker desktop application on windows.

6. **Configure Docker permissions (Optional):**
   If you don't want to use `sudo` with docker commands:
   ```bash
   sudo groupadd docker
   sudo usermod -aG docker $USER
...
ros2 launch wilson wilson_sim.launch.py
```
Please wait. It may take a minute or two to launch rviz and gazebo.
