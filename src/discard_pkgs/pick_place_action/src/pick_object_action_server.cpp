...
#include "pick_place_action/action/pick_object.hpp"

static const rclcpp::Logger LOGGER = rclcpp::get_logger("pick_object_action_server");
namespace mtc = moveit::task_constructor;

class PickObjectActionServer : public rclcpp::Node {
public:
    using PickObject = pick_place_action::action::PickObject;
    using GoalHandlePickObject = rclcpp_action::ServerGoalHandle<PickObject>;

    explicit PickObjectActionServer(const rclcpp::NodeOptions& options = rclcpp::NodeOptions())
        : Node("pick_object_action_server", options)
    {
        ...
        action_server_ = rclcpp_action::create_server<PickObject>(
            this,
            "pick_object",
            std::bind(&PickObjectActionServer::handle_goal, this, _1, _2),
            std::bind(&PickObjectActionServer::handle_cancel, this, _1),
            std::bind(&PickObjectActionServer::handle_accepted, this, _1));

        RCLCPP_INFO(LOGGER, "Pick Object Action Server started");
    }
...
private:
    ...
    {
        (void)uuid;
        RCLCPP_INFO(LOGGER, "Received goal request for object at position [%.3f, %.3f, %.3f] in frame '%s'",
    ...
    void handle_accepted(const std::shared_ptr<GoalHandlePickObject> goal_handle)
    {
        ...
        std::thread{std::bind(&PickObjectActionServer::execute, this, _1), goal_handle}.detach();
    }
    ...
    void execute(const std::shared_ptr<GoalHandlePickObject> goal_handle)
    {
        ...
        try {
            ...
            setupPlanningScene(target_point_out.point, goal->cylinder_height, goal->cylinder_radius);

            // Create and execute MTC task
            feedback->current_stage = "Planning pick task";
            ...
            result->success = true;
            result->message = "Successfully picked object";
            result->final_object_pose.position.x = target_point_out.point.x;
            result->final_object_pose.position.y = target_point_out.point.y;
            result->final_object_pose.position.z = target_point_out.point.z;
            result->final_object_pose.orientation.w = 1.0;
                           ...
                           float height, float radius)
    {
        ...
        float obj_height = (height > 0.0) ? height : 0.122;
        float obj_radius = (radius > 0.0) ? radius : 0.033;
        ...
        RCLCPP_INFO(LOGGER, "Added collision object at [%.3f, %.3f, %.3f] with height %.3f and radius %.3f",
                    position.x, position.y, position.z, obj_height, obj_radius);
    }
    ...
    mtc::Task createTask(const geometry_msgs::msg::Point& object_position,
                        [[maybe_unused]] const std::shared_ptr<GoalHandlePickObject>& goal_handle,
                        [[maybe_unused]] std::shared_ptr<PickObject::Feedback>& feedback)
    {
        mtc::Task task;
        task.stages()->setName("pick object task");
...
int main(int argc, char** argv)
{
    ...
    auto action_server_node = std::make_shared<PickObjectActionServer>(options);
