...
class AudioLoop:
    ...
    async def handle_tool_call(self, tool_call):
        ...
        for function_call in tool_call.function_calls:
            ...
            # Add SILENT scheduling for action tools to prevent auto-response
            if function_call.name == "send_action_goal":
                response_data["scheduling"] = "SILENT"
            ...
            try:
                    ...
                    if audio_playing:
                        ...
                        if not stream_still_active and self.audio_in_queue.empty():
                            if self.active_muting:
                                # Clear any mic data captured while muted (leaked audio)
                ...
                if not audio_playing:
                    if self.active_muting:
                        ...
                        # Small delay to ensure mic is fully muted (increased for PulseAudio latency)
                        await asyncio.sleep(0.3)

                        # Clear any mic data captured before mute took effect
                        while not self.out_queue.empty():
                            try:
                                self.out_queue.get_nowait()
                            except asyncio.QueueEmpty:
